...
> **로컬 우선 개인 프로젝트 허브 — 연구와 개발의 인큐베이터**

## 목적

이 앱은 연구와 개발 등의 프로젝트 인큐베이터(아이디어) 역할을 합니다.

**개발 프로젝트**의 경우, 아이디어 단계에서 바로 개발에 들어가기 위한 개발상세서, 계획서, 기능상세서 등 개발에 필요한 문서들을 생성함을 목표로 합니다.

**연구 프로젝트**의 경우, 바로 연구 프로세스에 들어가기 전까지 문제의식, 연구문제, 연구공백, 연구목적, 연구질문, 가설개발, 연구설계, 데이터수집 및 분석절차, 분석툴 선택 등을 충분히 풍부하게 다듬는 것을 목적으로 합니다.

이후 아이디어들이 충분히 문서가 모이고 구체화되었을 때, 실행 가능한 프로젝트로 승격시켜 더 정교하고 빠른 연구 및 개발 프로세스를 실행함에 있어 이를 관리하고 모니터링하는 것이 PM Master Local의 핵심 목적입니다.

## 개요

PM Master Local는 데이터베이스 없이 파일시스템과 직접 동기화하는 로컬 우선(local-first) 프로젝트 관리 도구입니다. `~/Projects/` 폴더 전체를 웹 브라우저에서 시각화하고 관리할 수 있으며, 폴더 구조 자체가 곧 프로젝트의 상태를 나타냅니다. 별도의 데이터베이스 서버를 설치하거나 클라우드 서비스에 가입할 필요가 없습니다. 모든 데이터는 로컬 JSON 파일로 저장되므로 개인 정보와 연구 데이터의 프라이버시가 완벽하게 보장됩니다.
...
PM Master Local는 소프트웨어 개발과 학술 연구 워크플로를 하나의 인터페이스에서 동시에 지원하도록 설계되었습니다. 개발 프로젝트에서는 이슈 트래커, 할일 칸반, 간트 차트, 마크다운 문서 편집기를 활용할 수 있고, 연구 프로젝트에서는 문헌 검토, 데이터 분석, 논문 작성의 전 과정을 체계적으로 관리할 수 있습니다. 프로젝트 유형을 Research, Development, Research+Development, Other로 분류하여 각 유형에 맞는 워크플로를 적용할 수 있습니다.
...
기존의 프로젝트 관리 도구들은 대부분 클라우드 기반으로 동작하며, 데이터를 외부 서버에 저장합니다. 이는 연구 데이터나 기밀 프로젝트를 다루는 개인 개발자 및 연구자에게 프라이버시 우려를 야기합니다. PM Master Local는 이러한 문제를 근본적으로 해결합니다. 텔레메트리 데이터를 수집하지 않으며, 제3자 서비스에 의존하지 않고, 구독료도 없습니다. 데이터는 언제나 로컬 머신에 남아 있으며, 단순한 파일 복사만으로 백업과 마이그레이션이 가능합니다. 프로젝트 폴더 하나를 통째로 복사하면 모든 이력과 문서가 함께 이동합니다.
...
| ![로컬 폴더](docs/screenshots/17-local-folder-structure.png) | | | |
| 로컬 폴더 구조 | | | |
...
| 기능 | 설명 |
|------|------|
| 인물 관리 | 연락처 카드 (이름/소속/역할/전문분야/관계), 연결 관계, 관련 인원에서 자동 생성; **카드 더블클릭 시 상세 모달** (대형 사진, 전체 정보, 이전/다음 탐색, 키보드 화살표 + Esc) |
...
| 항목 | 수치 |
|------|------|
| 개발 기간 | 2026-03-19 ~ 2026-03-20 (~8.5시간) |
| 총 코드량 | 16,182줄 (43파일) |
| 프론트엔드 | 11,767줄 / TypeScript + React 19 |
| 백엔드 | 4,415줄 / Python + FastAPI |
| 커밋 수 | 53개 |
| AI 비용 | ~$80-150 (Claude Opus 4) |
| 동일 규모 수동 개발 비용 | ~$16,000 (320시간 @ $50/hr) |

### 평가

**강점**: "파일시스템이 곧 데이터베이스"라는 명확한 철학. 칸반, 간트, 이슈, 할일, 문서, 서버 제어를 하나의 인터페이스에 통합한 높은 기능 밀도. 학술 연구와 소프트웨어 개발 워크플로를 동시에 지원하는 이중 목적 설계. 로컬 JSON 저장으로 완전한 프라이버시 보장.

**개선 영역**: 3,500줄 이상의 대형 컴포넌트 파일을 탭별로 분리 필요. 페이지당 50개 이상의 useState를 Zustand/Context로 그룹핑 필요. 빈 catch 블록에 분류된 에러 처리 필요. 회귀 방지를 위한 pytest + Playwright 테스트 코드 부재.

상세 평가 및 로드맵은 [docs/evaluation-and-roadmap.md](docs/evaluation-and-roadmap.md)를 참조하세요.
