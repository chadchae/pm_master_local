...
export default function DashboardPage() {
  ...
  const [collabFilter, setCollabFilter] = useState<"all" | "collaboration" | "personal">(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("pm_collabFilter");
      if (saved === "collaboration" || saved === "personal") return saved;
    }
    return "all";
  });
  ...
  const { filterImportance, filterUrgency, filterSeverity, filterDefault } = useHighlightFilter();
  ...
  useEffect(() => {
    localStorage.setItem("pm_collabFilter", collabFilter);
  }, [collabFilter]);
  ...
  const getTypeBadge = (type?: string) => {
    ...
    const colorMap: Record<string, string> = {
      ...
      "리뷰":    "bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300",
      "리딩":    "bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300",
      "학습":    "bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300",
      "강의":    "bg-rose-100 text-rose-700 dark:bg-rose-900 dark:text-rose-300",
      "기획":    "bg-teal-100 text-teal-700 dark:bg-teal-900 dark:text-teal-300",
      "운영":    "bg-sky-100 text-sky-700 dark:bg-sky-900 dark:text-sky-300",
      "관리":    "bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300",
      "협업":    "bg-pink-100 text-pink-700 dark:bg-pink-900 dark:text-pink-300",
      "사업":    "bg-lime-100 text-lime-700 dark:bg-lime-900 dark:text-lime-300",
    };
    return (
  ...
  // Card border style by type
  const getCardBorderStyle = (type?: string, importance?: string, urgency?: string, severity?: string) => {
    // Default mode: plain theme border
    if (filterDefault) return theme.cardBorder;
    // Highlight filter logic (priority: 중요도 > 긴급도 > 엄정함)
    if (filterImportance && parseInt(importance || "0") >= 5)
      return "border-[3px] border-yellow-300 dark:border-yellow-300";
    if (filterUrgency && urgency === "urgent")
      return "border-[3px] border-red-500 dark:border-red-400";
    if (filterSeverity && severity === "critical")
      return "border-[3px] border-purple-500 dark:border-purple-400";
    // Type-based borders — color matches tag color
    const TYPE_BORDER: Record<string, string> = {
      "개인": "border-[3px] border-emerald-400 dark:border-emerald-500",
      "연구": "border-cyan-400 dark:border-cyan-500",
      "개발": "border-orange-400 dark:border-orange-500",
      "리뷰": "border-yellow-400 dark:border-yellow-500",
      "리딩": "border-blue-400 dark:border-blue-500",
      "학습": "border-purple-400 dark:border-purple-500",
      "강의": "border-rose-400 dark:border-rose-500",
      "기획": "border-teal-400 dark:border-teal-500",
      "운영": "border-sky-400 dark:border-sky-500",
      "관리": "border-indigo-400 dark:border-indigo-500",
      "협업": "border-pink-400 dark:border-pink-500",
      "사업": "border-lime-400 dark:border-lime-500",
    };
    return type && TYPE_BORDER[type] ? TYPE_BORDER[type] : theme.cardBorder;
  };
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          {/* Collab filter — 협업/개인 toggle */}
          <div className="flex items-center gap-1 mr-2">
            {(["all", "collaboration", "personal"] as const).map((v) => (
              <button
                key={v}
                onClick={() => setCollabFilter(v)}
                className={`text-xs px-2 py-1 rounded-md transition-colors ${
                  collabFilter === v
                    ? v === "collaboration"
                      ? "bg-pink-100 text-pink-700 dark:bg-pink-900/40 dark:text-pink-300 font-medium"
                      : v === "personal"
                      ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300 font-medium"
                      : "bg-neutral-200 text-neutral-700 dark:bg-neutral-700 dark:text-neutral-300 font-medium"
                    : "text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300"
                }`}
              >
                {v === "all" ? "전체" : v === "collaboration" ? "협업" : "개인"}
              </button>
            ))}
          </div>
                      ...
                      <div
                        ...
                        className={`group p-2.5 rounded-lg border transition-all cursor-pointer relative ${
                          ...
                          draggedProject?.name === project.name
                            ...
                            : `${getCardBorderStyle(project.metadata?.유형, project.metadata?.["중요도"], project.metadata?.["긴급도"], project.metadata?.["위급도"])} ${theme.card} ${theme.cardHover}`
                        }`}
                      >
                        {getFocusBadge(project.name) && (
