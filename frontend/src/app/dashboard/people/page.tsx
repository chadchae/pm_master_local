...
export default function PeoplePage() {
  ...
  // Scroll to and highlight person when coming from external link
  useEffect(() => {
    if (!highlightedId || !highlightRef.current) return;
    highlightRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
    const timer = setTimeout(() => setHighlightedId(null), 3000);
    return () => clearTimeout(timer);
  }, [highlightedId, people]);
      ...
      {personList.map((person) => {
        ...
        return (
          <div
            ...
            className={isHighlighted ? "rounded-xl ring-2 ring-indigo-400 ring-offset-2 transition-all" : ""}
          >
            <PersonCard
              person={person}
              allPeople={people}
