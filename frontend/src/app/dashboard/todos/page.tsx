...
import { useFocusTasks } from "@/lib/focusTasks";
...
export default function TodosPage() {
  ...
  const { toggleWeeklyFocus, toggleMonthlyFocus, isWeeklyFocus, isMonthlyFocus } = useFocusTasks();
                            ...
                            <input
                              ...
                              onChange={() => {
                                if (col === "done") {
                                  ...
                                  if (isWeeklyFocus(todo.id)) toggleWeeklyFocus(todo.id, todo.title, todo.due_date || undefined);
                                  if (isMonthlyFocus(todo.id)) toggleMonthlyFocus(todo.id, todo.title, todo.due_date || undefined);
                                }
                              }}
                          ...
                          <div className="flex items-center gap-0.5 flex-shrink-0">
                            {selectedProject === PIN && (
                              <>
                                <button
                                  onClick={() => {
                                    const r = toggleWeeklyFocus(todo.id, todo.title, todo.due_date || undefined);
                                    if (r.blocked) toast.error("이번주 집중할일은 최대 3개입니다");
                                  }}
                                  className={`p-1 rounded transition-colors ${
                                    isWeeklyFocus(todo.id)
                                      ? "text-green-500"
                                      : "text-neutral-300 dark:text-neutral-600 opacity-0 group-hover:opacity-100 hover:text-green-400"
                                  }`}
                                  title="이번주 집중할일"
                                >
                                ...
                                <button
                                  onClick={() => {
                                    const r = toggleMonthlyFocus(todo.id, todo.title, todo.due_date || undefined);
                                    if (r.blocked) toast.error("이번달 집중할일은 최대 3개입니다");
                                  }}
                                  className={`p-1 rounded transition-colors ${
                                    isMonthlyFocus(todo.id)
                                      ? "text-blue-500"
                                      : "text-neutral-300 dark:text-neutral-600 opacity-0 group-hover:opacity-100 hover:text-blue-400"
                                  }`}
                                  title="이번달 집중할일"
                                >
                            ...
                            <button onClick={() => startEdit(todo)} className="p-1 text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300 rounded opacity-0 group-hover:opacity-100 transition-opacity"><Pencil className="w-3 h-3" /></button>
                            <button onClick={() => deleteTodo(todo.id)} className="p-1 text-neutral-400 hover:text-red-500 rounded opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="w-3 h-3" /></button>
                          </div>
                        </div>
                        {todo.description && (
