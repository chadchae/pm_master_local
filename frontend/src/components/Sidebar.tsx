...
import { MajorSchedulePanel } from "./MajorSchedulePanel";
...
export function Sidebar() {
  ...
  const [showMajorSchedule, setShowMajorSchedule] = useState(false);
    ...
    <>
      ...
      >
          ...
          {!collapsed && (
            <div className="flex flex-col min-w-0">
              <span className="text-sm font-semibold text-neutral-900 dark:text-white truncate">
                PM Master Local <span className="text-[10px] font-normal text-neutral-400">v2.2.0</span>
              </span>
        ...
        <div className="border-t border-neutral-200 dark:border-neutral-800 p-2 space-y-0.5">
          ...
          <button
            onClick={() => setShowMajorSchedule(true)}
            className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-cyan-600 dark:text-cyan-400 hover:bg-cyan-50 dark:hover:bg-cyan-950/30 hover:text-cyan-700 dark:hover:text-cyan-300 transition-colors w-full"
            title={collapsed ? "주요일정" : undefined}
          >
            <CalendarDays className="w-5 h-5 flex-shrink-0" />
            {!collapsed && <span>주요일정</span>}
          </button>
      ...
      <MajorSchedulePanel open={showMajorSchedule} onClose={() => setShowMajorSchedule(false)} />
    </>
  );
}
