...
const KNOWN_HTML_TAGS = new Set([
  ...
  "input","ins","kbd","label","legend","li","link","main","mark","menu","meta",
  "meter","nav","noscript","ol","optgroup","option","output","p","picture","pre",
...
function escapeUnknownHtmlTags(content: string): string {
  return content.replace(/<(\/?)\s*([a-zA-Z][a-zA-Z0-9-]*)([^>]*)>/g, (match, _slash, tag) => {
...
export function DocumentViewer({ selectedDoc, docContent, docBlobUrl, docHtml, showLineNumbers }: DocumentViewerProps) {
  ...
  if (selectedDoc?.endsWith(".md") || selectedDoc?.endsWith(".rmd") || selectedDoc?.endsWith(".qmd")) {
    ...
    const fixedContent = escapeUnknownHtmlTags(
      docContent.replace(/\\text\{([^}]*)}/g, (match, inner) =>
        "\\text{" + inner.replace(/(?<!\\)%/g, "\\%") + "}"
      )
    );
    return (
