...
export function TodoPanel({ open, onClose }: TodoPanelProps) {
  ...
  const toggleDone = async (project: string, todoId: string, title: string, dueDate?: string) => {
    // Remove from focus lists if selected
    if (isWeeklyFocus(todoId)) toggleWeeklyFocus(todoId, title, dueDate);
    if (isMonthlyFocus(todoId)) toggleMonthlyFocus(todoId, title, dueDate);
                      ...
                      <div className="pb-2">
                        {proj.items.map((todo) => (
                          ...
                          >
                            ...
                            <button
                              onClick={() => toggleDone(proj.project, todo.id, todo.title, todo.due_date || undefined)}
                            ...
                            {/* 집중할일 아이콘: PM Master Local 할일에만 표시 */}
                            {proj.project === PIN && (
                              <>
                            {/* 이번주 집중할일 토글 */}
                            <button
